...
function config(app, { showLoginPage, idpConfig, serviceProvider }) {
  for (const idp of ['singPass', 'corpPass']) {
    app.get(`/${idp.toLowerCase()}/metadata`, (req, res) => {
      ...
      if (idp === 'corpPass') {
        providerConfiguration['authorization-info_endpoint'] = `${baseUrl}/${idp.toLowerCase()}/authorization-info`
      }

      res.send(providerConfiguration)
    })
